...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    /* 복사 크기 = 기존 payload 와 요청 payload 중 작은 값 */
    size_t cpy_size = oldsize - DSIZE;
    if (size < cpy_size)
        cpy_size = size;

    /* 뒤 블록이 free 이고, 합치면 충분할 때 → 제자리 확장 */
    if (!GET_ALLOC(HDRP(next))) {
        ...
        if (newsize >= asize) {
            ...
            /* free list 에서 next 제거 */
            delete_free_list(find_index(nextsize), next);
            ...
            /* 남은 공간 split */
            size_t remain = newsize - asize;
            if (remain >= 2*DSIZE) {
                void *remain_bp = NEXT_BLKP(ptr);
                PUT(HDRP(remain_bp), PACK(remain, 0));
                PUT(FTRP(remain_bp), PACK(remain, 0));
                insert_free_list(find_index(remain), remain_bp);
            }
            else{
                PUT(HDRP(ptr), PACK(newsize, 1));
                PUT(FTRP(ptr), PACK(newsize, 1));
            }
            return ptr; //  memcpy 없이 그대로 반환
        }
    }
    /* 제자리 확장 불가능 → 새로 malloc */
